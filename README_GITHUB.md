# WONTECH 업무관리 V14.3.1 — GitHub Windows 빌드
...
- 저장소의 `package.json`을 열었을 때 버전이 `14.3.1`이고 `"dist": "electron-builder --win nsis --x64 --publish never"`가 보여야 합니다.
...
- 영문견적 수신담당자 선택·추가·편집 및 수신처·메일주소 자동 입력
- 영문견적 JPG/PDF 저장 시 입력값 중앙 정렬과 긴 영문 자동 줄바꿈
- 재고관리: 제품 사진, 고유번호, 단가, 재고/매입/불출수량, 불출 사유 및 이력관리
- 재고관리 저장: 현재 재고 목록 또는 불출 이력을 엑셀/JPG/PDF A4 가로형으로 저장
- ASAP관리: 제품 사진, 요청회사/담당자, 요청/확정금액, 미처리/완료 목록 자동 분류
